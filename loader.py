...
class SpeechLoader():
	...
	def __init__(self, wav_path=None, label_file=None, batch_size=1, n_mfcc=20, encoding='utf-8', st_size_limit=24000):
	...
	def preprocess(self, wav_path, label_file, wavs_file, vocab_file, mfcc_tensor, label_tensor):
		def handle_file(dirpath, filename):
			if filename.endswith('.wav') or filename.endswith('.WAV'):
				...
				if os.stat(filename_path).st_size < self.st_size_limit:
					return
	...
	def get_batches(self):
		for mfcc in self.mfcc_tensor:
			while len(mfcc) < self.wav_max_len:
				mfcc.append([0]*self.n_mfcc)
		for label in self.label_tensor:
			while len(label) < self.label_max_len:
				label.append(0)
		self.mfcc_tensor = np.array(self.mfcc_tensor, dtype=np.float32)
		self.label_tensor = np.array(self.label_tensor, dtype=np.int32)
		batches = [(self.mfcc_tensor[i], self.label_tensor[i]) for i in range(len(self.mfcc_tensor))]
		return batches
